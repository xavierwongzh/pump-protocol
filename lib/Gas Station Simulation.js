...
//audio
audio1 = "audio/gas.m4a";
audio2 = "audio/11. sanctuary.mp3";
audio3 = "audio/The Way of the RAIN.mp3";
audio4 = "audio/matcha latte.mp3";
audio5 = "audio/10. Silence before the storms.mp3";
var audioFiles = [audio1, audio2, audio3, audio4, audio5]; // Array of audio files
...
// var workerone = gas_station_worker[2]; // the worker1 is the third element of the gas_station_worker list.
// var workertwo = gas_station_worker[3]; // the worker2 is the fourth element of the gas_station_worker list.
// var workerthree = gas_station_worker[4]; // the worker3 is the fifth element of the gas_station_worker list.
...
// MAKE THIS DYNAMIC USER CAN INPUT
var probArrival = 0.2;
	...
	for (let i = 0; i < 40; i++) {
		for (let j = 0; j < 55; j++) {
			if ( ((i - centerI) ** 2) + ((j - centerJ) ** 2) < (radius ** 2) ) {
				// Skip this iteration if within the circle, leaving it empty.
				continue;
				}
			newareas.append("svg:image")
					...
					if (car.pumpedVolume >= car.targetVolume) {
						if (car.currentPumpDelay === undefined) {
							...
							if (!car.hasRecordedPumpStats) {
								...
								// Update your statistic (assume statistics[1] is pump duration and [3] is pump amount).
			...
			if (hasArrived) {
				...
				if (availableCashier) {
					...
					if (!collision) {
						...
						// Optionally, you might log or take other action if a collision is detected.
		...
		case PAYING:
			...
			if (hasArrived) {
				...
				if (Math.random() < probDeparture) {
					...
					// Free up the cashier. (We assume that the worker who is busy for this driver is unique.)
